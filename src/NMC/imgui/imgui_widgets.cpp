...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    ...
    {
        float width_rounded = ImFloor(items[id].Width);
        width_excess += items[id].Width - width_rounded;
        items[id].Width = width_rounded;
    }
    if (width_excess > 0.0f)
        for (int id = 0; id < count; id++)
            if (items[id].Index < (int)(width_excess + 0.01f))
                items[id].Width += 1.0f;
}
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
   ...
   // bool value_changed = false; -- > [Edited]
...
static void STB_TEXTEDIT_DELETECHARS(STB_TEXTEDIT_STRING* obj, int pos, int id)
{
    ImWchar* dst = obj->TextW.Data + pos;

    // We maintain our buffer length in both UTF-8 and wchar formats
    obj->Edited = true;
    obj->CurLenA -= ImTextCountUtf8BytesFromStr(dst, dst + id);
    obj->CurLenW -= id;

    // Offset remaining text (FIXME-OPT: Use memmove)
    const ImWchar* src = obj->TextW.Data + pos + id;
    while (ImWchar c = *src++)
        *dst++ = c;
    *dst = '\0';
}
    ...
    if (g.ActiveId == id)
    {
        ...
        if (io.InputQueueCharacters.Size > 0)
        {
            if (!ignore_char_inputs && !is_readonly && !user_nav_input_start)
                for (int id = 0; id < io.InputQueueCharacters.Size; id++)
                {
                    // Insert character if they pass filtering
                    unsigned int c = (unsigned int)io.InputQueueCharacters[id];
                    if (c == '\t' && io.KeyShift)
                        continue;
                    if (InputTextFilterCharacter(&c, flags, callback, callback_user_data))
                        state->OnKeyPressed((int)c);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int id = 0; id < components; id++)
        {
            if (id > 0)
                SameLine(0, style.ItemInnerSpacing.x);
            SetNextItemWidth((id + 1 < components) ? w_item_one : w_item_last);

            // FIXME: When ImGuiColorEditFlags_HDR flag is passed HS values snap in weird ways when SV values go below 0.
            if (flags & ImGuiColorEditFlags_Float)
            {
                value_changed |= DragFloat(ids[id], &f[id], 1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f, fmt_table_float[fmt_idx][id]);
                value_changed_as_float |= value_changed;
            }
            else
            {
                value_changed |= DragInt(ids[id], &i[id], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][id]);
            }
            if (!(flags & ImGuiColorEditFlags_NoOptions))
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        {
            const float a0 = (id)     /6.0f * 2.0f * IM_PI - aeps;
            const float a1 = (id+1.0f)/6.0f * 2.0f * IM_PI + aeps;
            ...
            ShadeVertsLinearColorGradientKeepAlpha(draw_list, vert_start_idx, vert_end_idx, gradient_p0, gradient_p1, col_hues[id], col_hues[id + 1]);
        }
...
{
    if (tab_id != 0)
        for (int id = 0; id < tab_bar->Tabs.Size; id++)
            if (tab_bar->Tabs[id].ID == tab_id)
                return &tab_bar->Tabs[id];
    return NULL;
}
...
ImGuiOldColumns* ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID id)
{
    ...
    for (int id = 0; id < window->ColumnsStorage.Size; id++)
        if (window->ColumnsStorage[id].ID == id)
            return &window->ColumnsStorage[id];
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    if (columns->Columns.Size == 0)
    {
        ...
        for (int id = 0; id < columns_count + 1; id++)
        {
            ImGuiOldColumnData column;
            column.OffsetNorm = id / (float)columns_count;
            columns->Columns.push_back(column);
        }
    }

    for (int id = 0; id < columns_count; id++)
    {
        // Compute clipping rectangle
        ImGuiOldColumnData* column = &columns->Columns[id];
        float clip_x1 = IM_ROUND(window->Pos.x + GetColumnOffset(id));
        float clip_x2 = IM_ROUND(window->Pos.x + GetColumnOffset(id + 1) - 1.0f);
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int id = 1; id < columns->Count; id++)
        {
            ImGuiOldColumnData* column = &columns->Columns[id];
            float x = window->Pos.x + GetColumnOffset(id);
            const ImGuiID column_id = columns->ID + ImGuiID(id);
        ...
        if (dragging_column != -1)
        {
            if (!columns->IsBeingResized)
                for (int id = 0; id < columns->Count + 1; id++)
                    columns->Columns[id].OffsetNormBeforeResize = columns->Columns[id].OffsetNorm;
